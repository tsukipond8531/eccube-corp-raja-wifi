...
{
    const WIFI_API_USAGE_HISTORY = 'https://kw.api.gonosen.work/api/getus';
    const WIFI_API_USED_DATES = 'https://kw.api.gonosen.work/api/getud';
    const WIFI_API_PRICE_BY_DAYS = 'https://kw.api.gonosen.work/api/total/price';

    const SESSION_USAGE_HISTORY = 'session_usage_history';
    const SESSION_USAGE_PRICE = 'session_usage_price';
    ...
    public function index(Request $request, $id = null, RouterInterface $router)
    {
                ...
                case 'register':
                    ...
                    if (!$flowResult->hasError() && $form->isValid()) {
                        ...
                        if ($TargetOrder->getId() && $OldStatus->getId() != $NewStatus->getId()) {
                            ...
                            // 返却済みに変更された場合は, 返却日をセットする
                            if ($NewStatus->getId() == OrderStatus::RETURNED) {
                                $TargetOrder->setReturnedDate(new \DateTime());
                            }
        ...
        $OrderStatus = $TargetOrder->getOrderStatus();
        $UsageHistory = NULL;
        $RealPrice = 0;

        if ( $OrderStatus->getId() == OrderStatus::RETURNED ) {
            $Shipping = $TargetOrder->getShippings()[0];

            $imei = $TargetOrder->getImei();
            $deliveredDate = $Shipping->getShippingDeliveryDate();
            $returnedDate = $TargetOrder->getReturnedDate();

            $UsageHistory = $this->getUsageHistory( $imei, $deliveredDate, $returnedDate );
            $RealPrice = $this->getRealPrice( $imei, $deliveredDate, $returnedDate );
        } else {
            if ( $this->session->has(self::SESSION_USAGE_HISTORY) ) {
                $UsageHistory = $this->session->get(self::SESSION_USAGE_HISTORY);
            }
            if ( $this->session->has(self::SESSION_USAGE_PRICE) ) {
                $RealPrice = $this->session->get(self::SESSION_USAGE_PRICE);
            }

            $this->session->remove(self::SESSION_USAGE_HISTORY);
            $this->session->remove(self::SESSION_USAGE_PRICE);
        }
    ...
    private function getUsageHistory( $imei, $deliveredDate, $returnedDate ) {
        if ( is_null( $imei ) || is_null( $deliveredDate ) || is_null( $returnedDate ) ) {
            return NULL;
        }

        $data = [
            'begin_date' => $deliveredDate->format('Y-m-d'),
            'end_date' => $returnedDate->format('Y-m-d'),
            'imei' => $imei,
        ];

        $ch = curl_init(self::WIFI_API_USAGE_HISTORY);
    ...
    private function getRealPrice( $imei, $deliveredDate, $returnedDate ) {
        if ( is_null( $imei ) || is_null( $deliveredDate ) || is_null( $returnedDate ) ) {
            return 0;
        }
        ...
        $ch = curl_init(self::WIFI_API_USED_DATES);
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, $data);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        ...
        foreach ((array) $result['data'] as $countryCode => $days) {
            $data[$countryCode] = $days;
        }

        $ch = curl_init(self::WIFI_API_PRICE_BY_DAYS);
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, $data);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        
        $result = (array) json_decode(curl_exec($ch));
        $RealPrice = $result['data'];
     ...
     * 使用履歴取得する.
     *
     * @Route("/%eccube_admin_route%/order/{id}/usage_history", requirements={"id" = "\d+"}, name="admin_order_usage_history", methods={"POST"})
     *
     * @param Request $request
     */
    public function orderUsageHistory(Request $request, $id = null) {
        if ( !$id ) {
        ...
        $imei = $Order->getImei();
        $deliveredDate = $Shipping->getShippingDeliveryDate();
        $returnedDate = $Order->getReturnedDate();

        if ( !$imei ) {
            $this->addError('「端末番号」を入力してください。', 'admin');
            return $this->redirectToRoute('admin_order_edit', [ 'id' => $id ]);
        }

        if ( !$deliveredDate ) {
            $this->addError('「お届け日」を入力してください。', 'admin');
            return $this->redirectToRoute('admin_order_edit', [ 'id' => $id ]);
        }

        if ( !$returnedDate ) {
            $this->addError('「返却日」を入力してください。', 'admin');
            return $this->redirectToRoute('admin_order_edit', [ 'id' => $id ]);
        }
        ...
        $this->session->set(self::SESSION_USAGE_HISTORY, $UsageHistory);
        $this->session->set(self::SESSION_USAGE_PRICE, $RealPrice);
