...
class CsvImportController extends AbstractCsvImportController
{
    ...
    /**
     * 「端末番号」・「発送用伝票番号」・「返却用伝票番号」をCSVでインポート
     *
     * @Route("/%eccube_admin_route%/order/terminal_csv_upload", name="admin_order_terminal_csv_import", methods={"GET", "POST"})
     *
     * @throws \Doctrine\DBAL\ConnectionException
     */
    public function csvTerminal(Request $request)
    {
        $form = $this->formFactory->createBuilder(CsvImportType::class)->getForm();
        $errors = [];
            ...
            if ($form->isValid()) {
                ...
                if (!empty($formFile)) {
                    ...
                    try {
                        ...
                        $this->loadTerminalCsv($csv, $errors);
    ...
    protected function loadTerminalCsv(CsvImportService $csv, &$errors)
    {
        $columnConfig = $this->getTerminalColumnConfig();
        ...
        foreach ($csv as $line => $row) {
            // 出荷IDがなければエラー
            if (!isset($row[$columnNames['order_no']])) {
                $error = trans('admin.common.csv_invalid_required', ['%line%' => $line + 1, '%name%' => $columnNames['order_no']]);
            ...
            /* @var Order $Order */
            $Order = is_numeric($row[$columnNames['order_no']]) ? $this->orderRepository->find($row[$columnNames['order_no']]) : null;

            // 存在しない出荷IDはエラー
            if (is_null($Order)) {
                $error = trans('admin.common.csv_invalid_not_found', ['%line%' => $line + 1, '%name%' => $columnNames['order_no']]);
                $this->addError($error, 'admin');
                $errors[] = $error;
            ...
            if (isset($row[$columnNames['terminal_no']])) {
                $Order->setTerminalNo($row[$columnNames['terminal_no']]);
            }

            if (isset($row[$columnNames['shipping_slip_no']])) {
                $Order->setShippingSlipNo($row[$columnNames['shipping_slip_no']]);
            }

            if (isset($row[$columnNames['return_slip_no']])) {
                $Order->setReturnSlipNo($row[$columnNames['return_slip_no']]);
            }
    ...
    protected function getTerminalColumnConfig()
    {
        return [
            'order_no' => [
                'name' => '受注番号',
