...
class ProductController extends AbstractController
{
     ...
     * カートに追加.
     *
     * @Route("/products/add_rental_cart/{id}", name="product_add_rental_cart", methods={"POST"}, requirements={"id" = "\d+"})
     */
    public function addRentalCart(Request $request, Product $Product)
    {
        // エラーメッセージの配列
        $errorMessages = [];
        if (!$this->checkVisibility($Product)) {
        ...
        $options = [
            ...
            'country_name_jp' => $request->get('country_name_jp'),
            'rent_from' => $request->get('rent_from'),
            'rent_to' => $request->get('rent_to'),
            'des_price' => $request->get('des_price'),
            'daily_price' => $request->get('daily_price'),
            'option_price' => $request->get('option_price'),
            'product_1_price' => $request->get('product_1_price'),
            'product_2_price' => $request->get('product_2_price'),
            'compound' => true,
        ];
        ...
        // $form = $this->createForm(AddCartType::class, $cartItem, $options);
        $form->handleRequest($request);
        ...
        // カートへ追加
        $this->cartService->addRentalProduct($addCartData['product_class_id'], $addCartData['quantity'], $options['option_no'], $options['country_name_jp'], $options['rent_from'], $options['rent_to'], $options['des_price'], $options['daily_price'], $options['option_price'], $options['product_1_price'], $options['product_2_price']);
