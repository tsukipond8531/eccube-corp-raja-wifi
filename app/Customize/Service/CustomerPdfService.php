...
use Eccube\Service\CustomerPdfService as BaseService;

class CustomerPdfService extends BaseService
{
    /**
    ...
    public function getPdfFileName()
    {
        ...
        if ($this->PageNo() == 1) {
            $this->downloadFileName = '利用明細書_'.$this->lastOrderId.'.pdf';
        }
    ...
    protected function renderOrdererInfo(\Eccube\Entity\Order $Order)
    {
        ...
        //文書タイトル（納品書・請求書）
        $this->SetFont(self::FONT_GOTHIC, '', 13);
        $this->lfText(25, 58, $line1, 10);
        $this->lfText(25, 64, $line2, 10);
        $this->lfText(25, 70, $line3, 10);
     ...
     * @param string $price
     */
    protected function renderPriceData($price)
    {
        $this->SetFont(self::FONT_SJIS, 'B', 14);
        $this->lfText(63, 123, $price, 14);
        $this->restoreFont();
    }
