...
class CartService
{
    ...
    protected function restoreCarts($cartItems)
    {
        foreach ($this->getCarts() as $Cart) {
            ...
            $pre_order_id = $Cart->getPreOrderId();
     ...
     * @param $ProductClass ProductClass 商品規格
     * @param $quantity int 数量
     * @param $option_no string index
     * @param $country string Country
     * @param $rent_from string From
     * @param $rent_to string To
     * @param $des_price string Des_price
     * @param $daily_price string To Daily_price
     * @param $option_price string Option_price
     * @param $product_1_price string Product1_price
     * @param $product_2_price string Product2_price
     *
     * @return bool 商品を追加できた場合はtrue
     */
    public function addRentalProduct($ProductClass, $quantity = 1, $option_no, $country, $rent_from, $rent_to, $des_price, $daily_price, $option_price, $product_1_price, $product_2_price)
    {
        if (!$ProductClass instanceof ProductClass) {
            $ProductClassId = $ProductClass;
            $ProductClass = $this->entityManager
        ...
        $newItem->setPrice($ProductClass->getPrice02IncTax());
        $newItem->setProductClass($ProductClass);
        $newItem->setOptionNo($option_no);
        $newItem->setCountryNameJP($country);
        $newItem->setRentFrom($rent_from);
        $newItem->setRentTo($rent_to);
        $newItem->setDesPrice($des_price);
        $newItem->setDailyPrice($daily_price);
        $newItem->setOptionPrice($option_price);
        $newItem->setProduct1Price($product_1_price);
        $newItem->setProduct2Price($product_2_price);
