...
class MailService
{
     ...
     * @param string $activateUrl アクティベート用url
     */
    public function sendOrderSelectedTemplate(Order $Order, $mailTemplateId)
    {
        log_info('一括送信メール送信開始');

        $MailTemplate = $this->mailTemplateRepository->find($mailTemplateId);
        ...
        log_info('一括送信メール送信完了', ['count' => $count]);
    ...
    public function sendNoticeOrderComplete(Order $Order, $complete_date)
    {
        log_info('レンタン完了通知送信開始');
        ...
        log_info('レンタン完了通知メール送信完了', ['count' => $count]);
